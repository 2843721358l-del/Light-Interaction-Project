...
if HAS_TRITON:
    # --------------------------------------------------------------------------
    # Kernel 1: Q Preparation (Tiling + Mean Pooling)
    # --------------------------------------------------------------------------
    @triton.jit
    ...
    # --------------------------------------------------------------------------
    # Kernel 2: KV Preparation (Double Stream Fused)
    # --------------------------------------------------------------------------
    @triton.jit
    def _fused_kv_prep_kernel(
        ...
        # Store K Mean
        mean_val = tl.sum(val_k.to(tl.float32), axis=0) / BLOCK_SIZE_FLOAT
        tl.store(mk_base + (blk_idx + pool_offset_blk) * m_n + offs_d * m_d, mean_val.to(val_k.dtype))

    # --------------------------------------------------------------------------
    # Kernel 3: Untile (Scatter back to linear)
    # --------------------------------------------------------------------------
    @triton.jit
...
def fast_generate_indices(q_pool, k_pool, text_len, grid_thw_q, grid_thw_k, params, device):
    ...
    elif metric == 'scaled_dot_product':
        sim = torch.matmul(q_pool, k_pool.transpose(-2, -1)) * (1.0 / math.sqrt(d))
    else:
    ...
    n_prot_blk = 0
    Nt_k, Nh, Nw = grid_thw_k[0], grid_thw_q[1], grid_thw_q[2]
    
    if params.get('recent_protect', 0) > 0:
        n_prot_blk = min(((params['recent_protect'] + tt - 1) // tt) * Nh * Nw, nk)
        if n_prot_blk > 0:
            score_for_ranking[..., -n_prot_blk:] += 1e6
            
    n_history_blk = max(0, nk - n_prot_blk)
    history_ratio = params.get('topk_ratio', 1.0)
    k_budget_history = int(n_history_blk * history_ratio)
    k_budget = n_prot_blk + k_budget_history
    k_budget = min(max(k_budget, 1 if nk > 0 else 0), nk)
    ...
    txt_indices = torch.arange(num_txt_blk, device=device, dtype=torch.int32).view(1, 1, 1, -1).expand(b, h, vis_indices.shape[2], -1)
    final_indices = torch.cat([txt_indices, vis_indices.int()], dim=-1).contiguous()
    final_counts = torch.full((b, h, vis_indices.shape[2]), final_indices.shape[-1], dtype=torch.int32, device=device)
...
def run_sparse_attention(q, k_txt, v_txt, k_vis, v_vis, block_idx, latent_hw, sparse_params):
    ...
    nh, nw = (LATENT_H + (th - LATENT_H % th) % th) // th, (LATENT_W + (tw - LATENT_W % tw) % tw) // tw
    T_q, T_k = vis_len_q // (LATENT_H * LATENT_W), vis_len_k // (LATENT_H * LATENT_W)
    nt_q, nt_k = (T_q + (tt - T_q % tt) % tt) // tt, (T_k + (tt - T_k % tt) % tt) // tt
    
    num_txt_blk = (text_len + (BLOCK_SIZE - text_len % BLOCK_SIZE) % BLOCK_SIZE) // BLOCK_SIZE
    num_vis_blk_q, num_vis_blk_k = nt_q * nh * nw, nt_k * nh * nw
